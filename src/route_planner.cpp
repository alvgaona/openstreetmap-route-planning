...
bool RoutePlanner::CompareNodesByFValue(const RouteModel::Node* a, const RouteModel::Node* b) {
 ...
 * Starts by adding the start_node into the open_list.
 * Advances node by node by calling NextNode() which gets the lowest f-valued neighbor.
 * When the search has reached the end_node, constructs final path that was found.
 */
